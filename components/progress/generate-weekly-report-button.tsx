...
export function GenerateWeeklyReportButton({
  ...
  useEffect(() => {
    ...
    let show = false;
    if (day === 0) {
      // Sunday: allow generation only if daily report for today exists
      const dailyKey = `daily-report-generated-${userId}-${date}`;
      show = window.localStorage.getItem(dailyKey) === 'true';
    } else if (day === 1) {
      // Monday: allow generation regardless of Sunday's daily status
      end.setUTCDate(end.getUTCDate() - 1);
      show = true;
    } else {
      // Other days: compute last Sunday range but hide button
