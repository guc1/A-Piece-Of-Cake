...
export default function HighlightableText({
  ...
  const handleRemoveHighlight = useCallback(
    (
      ...
      if (!editable || !eraseEnabled || isSaving) return;
      ...
      void removeHighlight({ highlightId, targetSlug, blockId });
    },
    [blockId, editable, eraseEnabled, isSaving, removeHighlight, targetSlug],
  );
  ...
  return (
    ...
    >
      {segments.map((segment, index) =>
        segment.color ? (
          <span
            key={segment.highlightId ?? index}
            className={clsx('rounded px-1 py-0.5', {
              'cursor-pointer': editable && eraseEnabled,
              'focus:outline-none focus:ring-2 focus:ring-orange-400 focus:ring-offset-1':
                editable && eraseEnabled,
            })}
            style={{
              backgroundColor: segment.color,
              color: '#111827',
            }}
            role={editable && eraseEnabled ? 'button' : undefined}
            tabIndex={editable && eraseEnabled ? 0 : undefined}
            onMouseDown={(event) => {
              if (!editable || !eraseEnabled) return;
              event.preventDefault();
            }}
            onClick={(event) => handleRemoveHighlight(event, segment.highlightId)}
            onKeyDown={(event) => {
              if (event.key === 'Enter' || event.key === ' ') {
                handleRemoveHighlight(event, segment.highlightId);
              }
            }}
            aria-label={
              editable && eraseEnabled ? 'Remove highlight' : undefined
            }
          >
            {segment.text}
          </span>
