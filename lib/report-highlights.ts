...
export async function deleteReportHighlight(
  userId: number,
  highlightId: number,
): Promise<ReportHighlight> {
  const id = Number.isFinite(highlightId) ? Math.floor(highlightId) : NaN;
  ...
  const [existing] = await db
    ...
    .from(reportHighlights)
    .where(
      and(eq(reportHighlights.id, id), eq(reportHighlights.userId, userId)),
    )
    .limit(1);
  if (!existing) {
