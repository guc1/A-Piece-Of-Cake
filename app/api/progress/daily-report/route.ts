...
export async function POST(req: NextRequest) {
  ...
  lines.push(
    `The Ethos / Life Thesis / Telos/rational that the user is trying to achieve in his life (his cake) is: ${
      ...
      dailyIngredients.length
        ? ' including the information of which ingredients the user added for that day so the Title\nShort description\nUsefulness (scale 1-100)\n\nWhat it is\nWhy used\nWhen used / situations\nTips.  for that ingredient.'
        : ''
    }`,
  );
  ...
  for (const act of activities) {
    ...
    if (act.flavors.length || act.subflavors.length) {
      lines.push('Flavours added by the user (so the info of them):');
  ...
  lines.push(
    "start by the review of the daily aim, and the review of the ingredients (if filled in, if not filled in it will say everywhere, user didn't leave feedback)",
  );
  ...
  for (const act of activities) {
    ...
    lines.push(`activity id p1an-blk-${act.id}-${userId} and time ${act.start}-${act.end} and title ${act.title}`);
