...
export default function IngredientsClient({
  ...
  function openImprove() {
    ...
    setImproveChatInput('');
    // close the edit dialog so the chat overlay isn't hidden
    setOpen(false);
    setImproveOpen(true);
  }
    ...
    try {
      ...
      if (data.response) {
        ...
        if (parsed && parsed.title) {
          ...
          if (ok) {
            // ensure usefulness is numeric; fall back to existing value
            if (parsed.usefulness !== undefined) {
              const u = Number(parsed.usefulness as any);
              if (!Number.isNaN(u)) parsed.usefulness = u as any;
              else delete parsed.usefulness;
            }
            ...
            // create a new ingredient from the AI suggestion then delete the old one
            const created = await createMine(fd);
            ...
            setForm({
              ...
              whyUsed: created.whyUsed,
              whenUsed: created.whenUsed,
              tips: created.tips,
              icon: created.icon,
            ...
            setImproveChatMessages([
              ...
              {
                role: 'assistant',
                content: `Updated ingredient "${created.title}".`,
              },
            ]);
          } else {
            setImproveChatMessages([
